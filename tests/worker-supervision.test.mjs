...
const jamesDetached = fs.readFileSync(
  "artifacts/api-server/src/routes/james-detached.ts",
  "utf8",
);
const supervision = fs.readFileSync(
  "artifacts/api-server/src/services/worker-supervision.ts",
  "utf8",
);
const taskSupervisor = fs.readFileSync(
  "artifacts/api-server/src/services/task-supervisor.ts",
  "utf8",
);
const executionControl = fs.readFileSync(
  "artifacts/api-server/src/services/task-execution-control.ts",
  "utf8",
);
const supervisionRoute = fs.readFileSync(
  "artifacts/api-server/src/routes/worker-supervision.ts",
  "utf8",
);
const runner = fs.readFileSync(
  "scripts/run-james-completion-review.sh",
  "utf8",
);
...
test("detached James execution gets a separate fresh supervisory pass", () => {
  ...
  assert.match(
    jamesDetached,
    /queueJamesCompletionReview\(taskId, "James Hermes", body\)/,
  );
  assert.match(
    supervision,
    /Perform a fresh verification pass even when James Hermes was also the executing worker/,
  );
});
...
test("continuous supervision backfills legacy Tasks", () => {
  assert.match(taskSupervisor, /ensureTaskWorkRequest\(\{/);
  assert.match(taskSupervisor, /executionRequestsCreated/);
});
test("owner escalation is idempotent", () => {
  assert.match(taskSupervisor, /alreadyEscalated/);
});
...
test("terminal supervision limit never resets retry counter", () => {
  ...
  assert.doesNotMatch(
    taskSupervisor,
    /Automatic supervision reached[\s\S]{0,700}supervisionAttempts:\s*0/,
  );
});
...
test("detached endpoint refuses to spend credits while circuit is open", () => {
  assert.match(
    jamesDetached,
    /task\.blocker\?\.startsWith\(CIRCUIT_BREAKER_PREFIX\)/,
  );
  assert.match(jamesDetached, /res\.status\(409\)/);
  assert.match(
    jamesDetached,
    /explicit recovery is required before another AI call/,
  );
});
...
test("James detached task and QA runners load the real Hermes profile credentials", () => {
  for (const source of [taskRunner, runner]) {
    assert.match(
      source,
      /JAMES_PROFILE_DIR=.*\/root\/\.hermes\/profiles\/james-hermes/,
    );
...
test("production deploy attaches the James profile env to the API service without printing secrets", () => {
  ...
  assert.match(
    deploy,
    /OPENROUTER_API_KEY presence verified without printing the secret/,
  );
});
...
test("production deploy persistently attaches the shared OpenRouter env to the OpenClaw gateway", () => {
  ...
  assert.match(
    deploy,
    /running OpenClaw gateway did not receive OPENROUTER_API_KEY/,
  );
  assert.match(
    deploy,
    /OpenClaw gateway environment attached and live process credential presence verified without printing the secret/,
  );
});

test("Task execution lifecycle remains harness-gated", () => {
