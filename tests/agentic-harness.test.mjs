...
const harness = await readFile(new URL("../artifacts/api-server/src/services/agentic-harness.ts", import.meta.url), "utf8");
const execution = await readFile(new URL("../artifacts/api-server/src/services/task-execution-control.ts", import.meta.url), "utf8");
const agents = await readFile(new URL("../AGENTS.md", import.meta.url), "utf8");
const docs = await readFile(new URL("../docs/mission-control/AGENTIC_OS_HARNESS.md", import.meta.url), "utf8");
...
test("completion is centrally gated by deterministic evals", () => {
  ...
  assert.match(execution, /if \(!evaluation\.passed\) return/);
  assert.match(execution, /advance\(refreshed, "completed", "Agentic harness evals passed/);
  for (const evalId of ["evidence_present", "completion_summary_present", "supervisor_verified", "no_unresolved_blockers", "protected_action_authorized"]) {
...
test("existing active executions self-migrate instead of being stranded", () => {
  ...
  assert.match(execution, /if \(contractFromRequirements\(request\.requirements\)\) return request/);
  assert.match(execution, /request = await ensureHarnessOnRequest\(request, task\)/);
});
...
test("future agents receive an explicit immutable execution workflow", () => {
  ...
  assert.match(agents, /Never weaken, bypass, delete or reinterpret a failed eval/);
