...
Use `sbb-finance-read latest-shift` for the most recent closed POS shift. Use `sbb-finance-read shift <uuid>` for a known shift or `sbb-finance-read date YYYY-MM-DD` for a Bangkok shift date. This tool is read-only, returns JSON with source provenance from both canonical POS tables and the matching SBB Dashboard daily record, and must be used before asking Cameron for figures that the tool can retrieve. Do not attempt direct database writes or expose its connection configuration.
