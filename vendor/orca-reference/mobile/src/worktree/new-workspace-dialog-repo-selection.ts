...
export function getMobileNewWorkspaceDialogEligibleRepos<T extends MobileNewWorkspaceDialogRepo>(repos: readonly T[]): T[] {
  return repos.filter((repo) => Boolean(repo.path))
}

function getMobileRepoExecutionHostId(repo: MobileNewWorkspaceDialogRepo): Exclude<ExecutionHostScope, 'all'> {
...
export function refreshMobileNewWorkspaceDialogSelectedRepo<T extends MobileNewWorkspaceDialogRepo>(repos: readonly T[], current: T | null): T | null {
  if (!current) {
    return null
  }
