...
export async function listSkills(filters: { name?: string | null; category?: string | null } = {}): Promise<SkillListResult> { const root = getSkillsRoot(); const files = await discoverLocalSkillFiles(root); const skills = await Promise.all(files.map(file => readMetadata(file, root))); const persisted = await readPersistedStatuses(root); const origins = sourceStatuses(skills, persisted); return { skills: filterSkills(skills, filters), origins, sources: origins }; }
