...
export async function getOperationalReadiness(): Promise<OperationalReadiness> {
  ...
  try {
    ...
    return {
      ...
      totals: { agents: 0, routableAgents: 0, healthyRuntimeInstances: 0, tasks: 0, workRequests: 0, pendingApprovals: 0 },
    };
  }

  try {
    const [agents, runtimeInstances, [taskCount], [workRequestCount], [approvalCount]] = await Promise.all([
    ...
    const healthyRuntimeInstances = runtimeInstances.filter(
      (runtime) => runtime.status === "running" && ["healthy", "ready", "online"].includes(runtime.health.toLowerCase()),
    );
    const pendingApprovals = await db.execute<{ count: number }>(sql`select count(*)::int as count from approvals where status = 'pending'`);
    const pendingApprovalCount = Number(pendingApprovals.rows[0]?.count ?? 0);
    ...
    checks.push({
      ...
      summary:
        runtimeInstances.length === 0
          ? "No managed runtime instances recorded; legacy/runtime-adapter workers may still operate"
    ...
    const criticalFailure = checks.some((check) => check.status === "fail");
    return {
      ...
      totals: {
        ...
        pendingApprovals: pendingApprovalCount || approvalCount?.count === 0 ? pendingApprovalCount : pendingApprovalCount,
      },
    };
  } catch {
