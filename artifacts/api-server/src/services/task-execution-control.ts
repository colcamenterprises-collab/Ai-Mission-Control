...
const TERMINAL_STATES = new Set<WorkRequestState>(["completed", "failed", "rejected", "cancelled"]);

export function taskExecutionKey(taskId: number): string { return `task:${taskId}:primary`; }

export async function latestTaskWorkRequest(taskId: number): Promise<WorkRequest | null> {
  const [request] = await db.select().from(workRequestsTable).where(eq(workRequestsTable.taskId, taskId)).orderBy(desc(workRequestsTable.updatedAt)).limit(1);
  return request ?? null;
}

async function ensureHarnessOnRequest(request: WorkRequest, task: typeof tasksTable.$inferSelect): Promise<WorkRequest> {
  if (contractFromRequirements(request.requirements)) return request;
  const contract = buildExecutionHarnessContract({ title: task.title, description: task.description, approvalRequired: task.approvalRequired });
  await db.update(workRequestsTable).set({ requirements: withExecutionHarnessRequirements(request.requirements, contract) }).where(eq(workRequestsTable.id, request.id));
  return (await latestTaskWorkRequest(task.id)) ?? request;
}

export async function ensureTaskWorkRequest(params: { task: typeof tasksTable.$inferSelect; agentId?: number | null; routingReason?: string | null }): Promise<WorkRequest> {
  const existing = await latestTaskWorkRequest(params.task.id);
  if (existing && !TERMINAL_STATES.has(existing.state as WorkRequestState)) return ensureHarnessOnRequest(existing, params.task);

  const riskLevel = params.task.approvalRequired ? 3 : 0;
  const approvalDecision = evaluateApproval({ riskLevel, agentCanAutoApprove: !params.task.approvalRequired, standingOwnerAuthority: false });
  const executionKey = existing ? `${taskExecutionKey(params.task.id)}:${Date.now()}` : taskExecutionKey(params.task.id);
  const harnessContract = buildExecutionHarnessContract({ title: params.task.title, description: params.task.description, approvalRequired: params.task.approvalRequired });

  const [created] = await db.insert(workRequestsTable).values({
    executionKey,
    ...
    requirements: withExecutionHarnessRequirements({ source: "canonical-task", ownerReviewRequired: params.task.ownerReviewRequired, taskStatus: params.task.status }, harnessContract),
    maxAttempts: 3,
    idempotencyClass: "side_effecting",
  }).onConflictDoNothing({ target: workRequestsTable.executionKey }).returning();

  if (!created) {
    const [stored] = await db.select().from(workRequestsTable).where(eq(workRequestsTable.executionKey, executionKey));
  ...
  let current = await transitionWorkRequest(created, "queued", { type: "orchestrator", id: "Mission Control", reason: "Canonical Task entered the execution control plane" });
  if (!params.agentId) return transitionWorkRequest(current, "blocked", { type: "router", reason: "UNASSIGNED: canonical Task has no executable worker yet" });
  if (approvalDecision === "AUTO_EXECUTE") return transitionWorkRequest(current, "approved", { type: "policy", reason: "Standing delegation permits ordinary Task execution" });

  current = await transitionWorkRequest(current, "awaiting_approval", { type: "policy", reason: `${approvalDecision} required` });
...
export async function markTaskExecutionCompleted(taskId: number, result: Record<string, unknown>): Promise<WorkRequest | null> {
  let request = await latestTaskWorkRequest(taskId);
  if (!request) return null;
  if (request.state === "approved" || request.state === "dispatched" || request.state === "acknowledged") request = (await markTaskExecutionRunning(taskId)) ?? request;
  if (request.state !== "running") return request;

  let contract = contractFromRequirements(request.requirements);
  if (!contract) {
    const [task] = await db.select().from(tasksTable).where(eq(tasksTable.id, taskId));
    if (!task) throw new Error(`Task #${taskId} not found while installing execution harness.`);
    request = await ensureHarnessOnRequest(request, task);
    contract = contractFromRequirements(request.requirements);
  }
  if (!contract) throw new Error(`Execution harness contract missing for Task #${taskId}; completion is fail-closed.`);

  const evaluation = evaluateCompletionContract({ contract, result, approvalDecision: request.approvalDecision });
  ...
  await db.update(workRequestsTable).set({
    result: { ...result, agenticHarness: { passed: evaluation.passed, evals: evaluation.evals } },
    ownerReport: typeof result.summary === "string" ? result.summary : null,
  }).where(eq(workRequestsTable.id, request.id));
  if (!evaluation.passed) return (await latestTaskWorkRequest(taskId)) ?? request;
