...
function safeApiKeyHint(apiKey: string | null | undefined): {
  hint: string | null;
  credentialHealth: "ok" | "missing" | "unreadable";
} {
  if (!apiKey) return { hint: null, credentialHealth: "missing" };
  try {
    return { hint: maskApiKey(decryptSecret(apiKey)), credentialHealth: "ok" };
  } catch {
    return { hint: null, credentialHealth: "unreadable" };
  }
}

function maskAgentForResponse(agent: typeof agentsTable.$inferSelect) {
  const { apiKey, ...rest } = agent;
  const credential = safeApiKeyHint(apiKey);
  return {
    ...rest,
    apiKeyHint: credential.hint,
    credentialHealth: credential.credentialHealth,
...
router.post(
  "/agents",
  createRateLimit("admin-write", 40, 60_000),
  async (req, res): Promise<void> => {
    ...
    const parsed = CreateAgentBody.safeParse({
      ...req.body,
      department: LEGACY_DEPARTMENT_PLACEHOLDER,
    });
    if (!parsed.success) {
...
router.put(
  ...
  async (req, res): Promise<void> => {
    ...
    if (
      ...
      !req.body.skills.every((skill: unknown) => typeof skill === "string")
    ) {
    ...
    const assignedSkills = await setAssignedSkillNamesForAgent(
      id,
      agent.name,
      req.body.skills,
    );
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    const parsed = UpdateAgentBody.safeParse(
      hasDepartment
        ? { ...req.body, department: LEGACY_DEPARTMENT_PLACEHOLDER }
        : req.body,
    );
    if (!parsed.success) {
      res.status(400).json({ error: parsed.error.message });
