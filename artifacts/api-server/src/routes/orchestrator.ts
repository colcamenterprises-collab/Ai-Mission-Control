...
router.post("/orchestrator/intake", async (req, res): Promise<void> => {
  ...
  if (assignedAgent) {
    ...
    if (isRuntimeConfigured(assignedAgent)) {
      ...
      await db.update(agentsTable).set({ currentTask: runtimeResult.ok ? null : `Task #${task.id}: ${title}`, status: runtimeResult.ok ? (isJamesHermes(assignedAgent) ? "active" : "idle") : "error", lastActive: runtimeResult.ok ? "work completed with playbooks" : "runtime failed", lastPing: runtimeResult.ok ? new Date() : assignedAgent.lastPing, tasksCompleted: runtimeResult.ok ? assignedAgent.tasksCompleted + 1 : assignedAgent.tasksCompleted }).where(eq(agentsTable.id, assignedAgent.id));
      await db.insert(activityTable).values({ agentName: assignedAgent.name, action: runtimeResult.ok ? "Completed orchestrated work with playbooks" : "Runtime failed orchestrated work", detail: runtimeResult.output ?? runtimeResult.error, status: runtimeResult.ok ? "active" : "error" });
    } else {
