...
router.post("/james/task-job", async (req, res): Promise<void> => {
  ...
  if (!message || !Number.isInteger(taskId) || taskId <= 0) { res.status(400).json({ error: "message and valid taskId are required" }); return; }
  ...
  try {
    ...
    await db.insert(taskMessagesTable).values({ taskId, author: "Mission Control", body: `BLOCKED — James detached worker could not be launched: ${error instanceof Error ? error.message : "unknown error"}` });
  ...
  await db.insert(taskMessagesTable).values({ taskId, author: "James Hermes", body: `Detached execution started. Job ${safeJobId}. Work will continue independently of Mission Control API restarts.` });
...
router.post("/james/report", async (req, res): Promise<void> => {
  ...
  const body = completed ? (output || `James completed detached job ${jobId}.`) : `BLOCKED — James detached job ${jobId} failed.${error ? ` ${error}` : ""}${output ? `\n\n${output}` : ""}`;
  await db.insert(taskMessagesTable).values({ taskId, author: "James Hermes", body });
  await db.update(tasksTable).set({ status: completed ? "review" : "blocked", unreadMessages: 1 }).where(eq(tasksTable.id, taskId));
  if (commandId && Number.isInteger(commandId)) {
    await db.update(agentCommandsTable).set({ acknowledgedAt: new Date(), deliveredViaHttp: completed }).where(eq(agentCommandsTable.id, commandId));
  }
  await db.insert(activityTable).values({ agentName: "James Hermes", action: completed ? "Detached worker completed task" : "Detached worker failed task", detail: `Task #${taskId}; job ${jobId}`, status: completed ? "pending" : "error" });
  await db.insert(taskMessagesTable).values({ taskId, author: "Mission Control", body: completed ? "WORK COMPLETE — orchestrator review recorded. Owner final sign-off is required before this task is archived." : "FOLLOW-UP REQUIRED — the task remains blocked until the execution failure is resolved." });
