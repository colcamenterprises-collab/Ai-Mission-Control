...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const visibleNavItems = useMemo(
    () =>
      navItems.filter((i) => i.label.toLowerCase().includes(normalizedQuery)),
    [normalizedQuery],
  );
  useEffect(() => {
    const q = window.matchMedia("(max-width: 1180px)");
    const sync = (e: MediaQueryListEvent) => setIsCollapsed(e.matches);
  ...
  function renderNavItem(item: NavItem) {
    ...
    return (
      <Link
        ...
        className={`mission-nav-item ${active ? "mission-nav-active" : ""}`}
      >
  ...
  return (
    <div className="mission-app-bg relative min-h-screen overflow-hidden flex text-foreground">
      ...
      <aside
        className={`mission-sidebar relative z-10 flex-shrink-0 flex flex-col ${isCollapsed ? "mission-sidebar-collapsed" : "mission-sidebar-expanded"}`}
      >
