    ...
    <div className="mb-4 overflow-hidden rounded-lg border border-border">
      <table className="w-full text-left text-xs">
        <thead className="bg-secondary/50 font-mono uppercase text-muted-foreground">
          <tr><th className="p-2">Repo</th><th className="p-2">Branch</th><th className="p-2">Last checked</th><th className="p-2">Last imported commit</th><th className="p-2">Import status</th><th className="p-2">Skill count</th><th className="p-2">Error</th></tr>
        </thead>
        <tbody>
          {sources.map((source) => (
    ...
    <div className="space-y-2">
      {skills.map((skill) => (
        ...
        >
          ...
          <p className="mt-1 font-mono text-[11px] text-muted-foreground">Type: {skill.source.localStatus} · Status: available · Origin: {skill.source.sourceRepo ?? "NULL"}</p>
        </button>
      ))}
    </div>
  );
    ...
    <div className="workspaces-shell flex h-full flex-col overflow-y-auto">
      <div className="workspaces-canvas flex-1 space-y-6">
        <header>
          ...
          <p className="mt-3 max-w-2xl text-sm text-muted-foreground">Agents consume local SKILL.md files from the Mission Control skill library. External repositories are import origins only and are not runtime dependencies.</p>
        </header>

        <section className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(18rem,24rem)_1fr]">
          <div className="workspace-panel p-4">
            <div className="mb-4 flex items-center justify-between gap-3">
              <h2 className="dashboard-section-title flex items-center gap-2"><BookOpen className="h-4 w-4" /> Local Skills</h2>
            ...
            {isLoading ? (
              ...
              <>
                <LocalSkillList skills={skills} selectedId={selectedSkillId} onSelect={setSelectedId} />
                <h2 className="dashboard-section-title mt-6 mb-3">Import Sources / Origins</h2>
                <OriginStatusTable sources={origins} />
              </>
            )}
          </div>
