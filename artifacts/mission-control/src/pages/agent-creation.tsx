...
export default function AgentCreation() {
  ...
  return <div className="employee-factory-shell"><div className="employee-factory-canvas">
    ...
    <div className="employee-factory-grid"><main className="employee-factory-main">
      <section className="employee-factory-card">
        ...
        <div className="employee-avatar-row">
          <label className="employee-avatar-upload"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={event => uploadAvatar(event.target.files?.[0])} />{avatarDataUrl ? <img src={avatarDataUrl} alt="Employee preview" /> : <ImagePlus />}<span>{avatarDataUrl ? "Change photo" : "Add employee photo"}</span></label>
          <div className="employee-avatar-copy"><strong>Employee photo</strong><span>Optional. PNG, JPG or WebP, under 1 MB.</span></div>
        </div>
    ...
    <aside className="employee-factory-sidebar">
      <section className="employee-factory-card employee-preview-card">{avatarDataUrl ? <img className="employee-preview-avatar employee-preview-photo" src={avatarDataUrl} alt="Employee" /> : <div className="employee-preview-avatar"><Bot /></div>}<span className="employee-preview-eyebrow">New employee</span><h2>{form.name || "Unnamed employee"}</h2><p>{form.role || "Add a job title"}</p><div className="employee-preview-meta"><span><BriefcaseBusiness />{selectedProject?.name || "Project required"}</span><span><Cpu />{runtimeChoice === "openclaw" ? "OpenClaw" : runtimeChoice === "hermes" ? "Hermes" : "Existing agent"}</span><span><ShieldCheck />0 Mission Control skills</span></div></section>
