...
export default function TasksV2() {
  ...
  useEffect(() => {
    void Promise.all([refreshTasks(), refreshInbox(), refreshProjects()]).catch((error) => setBoardError(error instanceof Error ? error.message : "Unable to load Kanban")).finally(() => setLoading(false));
  ...
  const tasksByColumn = useMemo(() => Object.fromEntries(COLUMNS.map((column) => [column.id, tasks.filter((task) => column.matches.includes(task.status as never)).sort(newestFirst)])) as Record<ColumnId, TaskMeta[]>, [tasks]);
  ...
  function handleDragCancel(_event: DragCancelEvent) { setActiveTask(null); }
  ...
  return (
    <div className="mc-task-page mc-kanban-v2">
      ...
      <TaskDetailModal task={selectedTask} onClose={() => setSelectedTask(null)} onChanged={async () => { await refreshTasks(); }} />
    </div>
  );
}
...
function TaskCardVisual({ task, onOpen, dragProps, overlay = false }: { task: TaskMeta; onOpen?: () => void; dragProps?: Record<string, unknown>; overlay?: boolean }) {
  const dueDate = formatDueDate(task.dueDate);
  const approval = needsApproval(task);
  const tone = Math.abs(Number(task.id)) % 4;
  return <article className={`mc-task-card mc-task-card-tone-${tone} mc-task-agent-${agentTone(task.assignee)} ${approval ? "mc-task-card-approval" : ""} ${overlay ? "mc-kanban-v2-overlay-card" : ""}`} onClick={onOpen} {...dragProps}>
...
function InboxLane({ items, onChanged }: { items: InboxItem[]; onChanged: () => void }) {
  async function update(id: number, path: string, body?: object) { await fetch(`/api/inbox/${id}${path}`, { method: path ? "POST" : "PATCH", headers: authHeaders(), body: body ? JSON.stringify(body) : undefined }); onChanged(); }
...
function CreateTaskModal({ open, projects, onClose, onCreated }: { open: boolean; projects: Project[]; onClose: () => void; onCreated: () => void }) {
  ...
  async function submit() {
    setError(""); if (!form.title.trim() || !form.description.trim() || !project) { setError("Task title, description and project are required."); return; }
    setBusy(true);
    try {
      if (form.project === "__new") { const p = await fetch("/api/projects", { method: "POST", headers: authHeaders(), body: JSON.stringify({ name: project }) }); if (!p.ok && p.status !== 409) throw new Error("Unable to create project"); }
      ...
      setForm({ title: "", description: "", date: "", time: "", recurrence: "one_off", project: "Mission Control", newProject: "", approvalRequired: false, ownerReviewRequired: false }); onCreated();
...
function CreateNoteModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: () => void }) {
  ...
  if (!open) return null;
  async function save() { if (!content.trim()) { setError("Note content is required."); return; } setBusy(true); const response = await fetch("/api/inbox", { method: "POST", headers: authHeaders(), body: JSON.stringify({ title: title.trim() || null, content, source: "typed", createdBy: "Owner" }) }); setBusy(false); if (!response.ok) { setError("Unable to save idea."); return; } setTitle(""); setContent(""); onCreated(); }
...
function TaskDetailModal({ task, onClose, onChanged }: { task: TaskMeta | null; onClose: () => void; onChanged: () => void }) {
  const [details, setDetails] = useState<TaskDetails | null>(null); const [message, setMessage] = useState(""); const [note, setNote] = useState(""); const [checked, setChecked] = useState(false); const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  useEffect(() => { setDetails(null); setError(""); setNote(""); setChecked(false); if (!task) return; let cancelled = false; const load = async () => { const response = await fetch(`/api/tasks/${task.id}/details`, { headers: authHeaders(), cache: "no-store" }); if (response.ok && !cancelled) setDetails(await response.json() as TaskDetails); }; void load(); return () => { cancelled = true; }; }, [task]);
  if (!task) return null;
  const value = details ?? ({ ...task, messages: [] } as TaskDetails);
  async function action(path: string, body: object = {}) { setBusy(true); setError(""); try { const response = await fetch(`/api/tasks/${task.id}/${path}`, { method: "POST", headers: authHeaders(), body: JSON.stringify(body) }); if (!response.ok) { const result = await response.json().catch(() => ({})) as { error?: string }; throw new Error(result.error || `Unable to ${path}`); } await onChanged(); onClose(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Action failed"); } finally { setBusy(false); } }
  async function requestChanges() { if (!note.trim()) { setError("Add a factual change request first."); return; } await action("request-changes", { note }); }
  async function accept() { setBusy(true); setError(""); try { const response = await fetch(`/api/tasks/${task.id}/accept`, { method: "POST", headers: authHeaders(), body: JSON.stringify({ note }) }); if (!response.ok) throw new Error("Unable to accept work"); const archive = await fetch(`/api/tasks/${task.id}/archive`, { method: "POST", headers: authHeaders(), body: "{}" }); if (!archive.ok) throw new Error("Accepted, but archive failed"); await onChanged(); onClose(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to accept work"); } finally { setBusy(false); } }
  async function send() { if (!message.trim()) return; setBusy(true); const response = await fetch(`/api/tasks/${task.id}/messages`, { method: "POST", headers: authHeaders(), body: JSON.stringify({ body: message.trim() }) }); setBusy(false); if (response.ok) { setMessage(""); const fresh = await fetch(`/api/tasks/${task.id}/details`, { headers: authHeaders() }); if (fresh.ok) setDetails(await fresh.json() as TaskDetails); await onChanged(); } }
  return <Modal className="mc-task-detail-modal mc-task-timeline-modal" onClose={onClose} label={value.title}><header className="mc-task-modal-header"><span className="mc-task-modal-kicker">{columnFor(String(value.status))?.label ?? value.status}</span><h2>{value.title}</h2></header><div className="mc-task-detail-layout mc-task-timeline-layout"><main className="mc-task-summary-pane"><p className="mc-task-detail-description">{value.description || "No description provided."}</p><dl className="mc-task-detail-meta"><div><dt>Agent</dt><dd>{value.assignee || "Orchestrator"}</dd></div><div><dt>Project</dt><dd>{value.project || "Mission Control"}</dd></div><div><dt>Due</dt><dd>{formatDueDate(value.dueDate) || "Not set"}</dd></div><div><dt>Status</dt><dd>{String(value.status)}</dd></div></dl>{value.report && <section className="mc-task-detail-section"><h3>Agent Report</h3><p>{value.report}</p></section>}</main><aside className="mc-task-conversation mc-task-timeline"><header><JamesAvatar className="mc-task-conversation-avatar" /><div><h3>Task Timeline & Notes</h3><span>Permanent task record</span></div></header><div className="mc-task-conversation-messages mc-task-timeline-items">{value.messages.length ? value.messages.map((item) => <article key={item.id} className="mc-task-timeline-item mc-task-timeline-system"><div className="mc-task-timeline-topline"><span className="mc-task-timeline-event">NOTE</span><time>{new Date(item.createdAt).toLocaleString()}</time></div><strong>{item.author}</strong><p>{item.body}</p></article>) : <p className="mc-task-conversation-empty">No timeline entries yet.</p>}
  ...
  </div><div className="mc-task-conversation-compose"><textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={2} placeholder="Add a note inside this task" /><button onClick={() => void send()} disabled={busy || !message.trim()}><Send /></button></div></aside></div>{error && <p className="mc-task-form-error mc-task-action-error">{error}</p>}</Modal>;
}
