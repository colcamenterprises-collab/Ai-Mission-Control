    ...
    <div className="flex flex-col h-full">
      <div className="p-6 border-b border-border flex items-center justify-between">
        <div className="flex items-center gap-4">
          <h1 className="text-xl font-mono font-semibold uppercase tracking-tight">Calendar</h1>
          <div className="flex items-center gap-2">
            <button onClick={() => navigate(-1)} className="p-1 rounded hover:bg-secondary"><ChevronLeft className="w-4 h-4" /></button>
            <span className="font-mono text-sm w-44 text-center">{formatMonthYear(currentDate)}</span>
            <button onClick={() => navigate(1)} className="p-1 rounded hover:bg-secondary"><ChevronRight className="w-4 h-4" /></button>
          </div>
        </div>
          ...
          <div className="flex bg-secondary rounded overflow-hidden border border-border">
            {(["month", "week", "day"] as ViewMode[]).map(v => (
              <button key={v} onClick={() => setView(v)}
                className={`px-3 py-1.5 text-xs font-mono capitalize ${view === v ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
              >{v}</button>
            ))}
          </div>
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        <div className="flex gap-4 mb-4">
          {Object.entries(CATEGORY_LABEL_COLORS).map(([cat, color]) => (
            <div key={cat} className="flex items-center gap-1.5 text-xs">
              <div className={`w-2 h-2 rounded-full ${CATEGORY_COLORS[cat]}`} />
              <span className={`${color} capitalize`}>{cat}</span>
            </div>
          ))}
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {days.map((date, i) => {
                ...
                return (
                  <div key={i} className={`min-h-24 rounded border p-1.5 ${isCurrentMonth ? "border-border bg-card/30" : "border-border/30 bg-transparent opacity-40"} ${isToday ? "border-primary/50" : ""}`}>
                    ...
                    <div className="mt-1 space-y-1">
                      {dayEvents.slice(0, 3).map(ev => (
                        <div
                          key={ev.id}
                          className={`text-xs px-1 py-0.5 rounded truncate cursor-pointer ${CATEGORY_COLORS[ev.category]} bg-opacity-20 ${CATEGORY_LABEL_COLORS[ev.category]}`}
      ...
      <Dialog open={!!selectedEvent} onOpenChange={o => !o && setSelectedEvent(null)}>
        <DialogContent className="max-w-md bg-card border-border">
          <DialogHeader>
            <DialogTitle className="font-mono">{selectedEvent?.title}</DialogTitle>
          </DialogHeader>
          {selectedEvent && (
            <div className="space-y-3 text-sm">
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${CATEGORY_COLORS[selectedEvent.category]}`} />
                <span className={`capitalize ${CATEGORY_LABEL_COLORS[selectedEvent.category]}`}>{selectedEvent.category}</span>
              </div>
              <div>
                <label className="text-xs text-muted-foreground font-mono uppercase">Start</label>
                <p className="mt-1 font-mono">{new Date(selectedEvent.startDate).toLocaleString()}</p>
              </div>
              {selectedEvent.endDate && (
                <div>
                  <label className="text-xs text-muted-foreground font-mono uppercase">End</label>
                  <p className="mt-1 font-mono">{new Date(selectedEvent.endDate).toLocaleString()}</p>
                </div>
              )}
              {selectedEvent.description && <p className="text-muted-foreground">{selectedEvent.description}</p>}
              <div className="flex gap-2 pt-2">
                <Button variant="destructive" size="sm" onClick={() => {
                  deleteEvent.mutate({ id: selectedEvent.id }, { onSuccess: () => { invalidate(); setSelectedEvent(null); } });
                }}>Delete</Button>
...
function CreateEventDialog({ open, onClose, invalidate }: { open: boolean; onClose: () => void; invalidate: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={o => !o && onClose()}>
      <DialogContent className="max-w-md bg-card border-border">
        <DialogHeader>
          <DialogTitle className="font-mono">New Event</DialogTitle>
        </DialogHeader>
        <div className="space-y-3 text-sm">
          ...
          <Select value={form.category} onValueChange={v => setForm(f => ({ ...f, category: v }))}>
            ...
            <SelectContent>
              {["task","content","meeting","automation"].map(c => <SelectItem key={c} value={c} className="capitalize">{c}</SelectItem>)}
            </SelectContent>
          </Select>
          ...
          <div className="flex gap-2">
            <Button size="sm" onClick={handleSubmit} disabled={!form.title || !form.startDate}>Create</Button>
