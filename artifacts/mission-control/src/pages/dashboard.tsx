...
type OperationsBrief = {
  ...
  sections: {
    ...
    runningNow: unknown[];
    blocked: unknown[];
    topPriorities: Array<{ id: number; title: string }>;
    shippedSinceLastBrief: unknown[];
...
export default function Dashboard() {
  ...
  const waitingTasks =
    ...
    tasks.filter((task) => ["backlog", "ready", "review"].includes(task.status))
      .length;
      ...
      tasks
        .filter(
          (task) =>
            ...
            Boolean(task.recurrence && task.recurrence !== "one_off"),
        )
        .sort((a, b) => {
      ...
      <div className="mission-canvas mission-home-canvas mission-operations-home">
        ...
        <div className="mission-ops-grid mission-ops-grid-primary">
            ...
            <div className="mission-task-list">
              {latestTasks.length === 0 ? (
                ...
                latestTasks.map((task) => (
                  <Link
                    href="/tasks"
                    className="mission-task-row"
                    key={task.id}
                  >
                    <span
                      className={`mission-status-dot mission-status-${task.status}`}
                    />
                    <div>
          ...
          <article className="mission-panel mission-briefing-panel">
            ...
            <div className="mission-briefing-copy">
              <p>
                {activeTasks > 0
                  ? `${activeTasks} task${activeTasks === 1 ? " is" : "s are"} being worked now.`
                  : "No tasks are actively running right now."}
              </p>
              <p>
                {approvals.length > 0
                  ? `${approvals.length} item${approvals.length === 1 ? " needs" : "s need"} your approval before work can continue.`
                  : "Nothing is waiting for owner approval."}
              </p>
              <p>
                {blockedTasks > 0
                  ? `${blockedTasks} task${blockedTasks === 1 ? " is" : "s are"} blocked and should be reviewed.`
                  : `${waitingTasks} task${waitingTasks === 1 ? " is" : "s are"} queued or waiting.`}
              </p>
              <p>
                {automations.length > 0
                  ? `${automations.length} scheduled or recurring item${automations.length === 1 ? " is" : "s are"} visible in the upcoming queue.`
                  : "No upcoming automation is currently visible from task scheduling."}
              </p>
              <p>
                {briefQuery.data
                  ? `${briefQuery.data.sections.shippedSinceLastBrief.length} execution${briefQuery.data.sections.shippedSinceLastBrief.length === 1 ? "" : "s"} shipped in the last 24 hours; ${briefQuery.data.sections.risks.length} evidence-backed risk signal${briefQuery.data.sections.risks.length === 1 ? "" : "s"} remain open.`
                  : "Execution brief is unavailable until Mission Control data loads."}
              </p>
              <p>
                <strong>Recommended next action:</strong>{" "}
                {briefQuery.data?.sections.recommendedNextAction ?? "UNKNOWN"}
              </p>
            </div>
            <Link href="/tasks" className="mission-briefing-action">
              Open workboard
            </Link>
          </article>
        </div>
        ...
        <div className="mission-ops-grid mission-ops-grid-secondary">
          ...
          <OpsPanel title="Live activity" action="Run history" href="/reports">
            {latestTasks.length === 0 ? (
              <CompactEmpty>No activity yet</CompactEmpty>
            ) : (
              latestTasks
                .slice(0, 4)
                .map((task) => (
                  <OpsRow
                    key={task.id}
                    title={task.title}
                    meta={`${prettyStatus(task.status)} · ${timeAgo(task.updatedAt)}`}
                    href="/tasks"
                    accent="activity"
                  />
                ))
          ...
          >
            {automations.length === 0 ? (
              <CompactEmpty>No scheduled work</CompactEmpty>
            ) : (
              automations.map((task) => (
                <OpsRow
                  ...
                  meta={
                    task.dueDate
                      ? formatSchedule(task.dueDate, task.recurrence)
        ...
        <button
          className="mission-command-trigger"
          type="button"
...
function PanelTitle({
  ...
  return (
    <div className="mission-panel-title">
      ...
      <Link href={href} className="mission-text-action">
        {action}
      </Link>
    </div>
  );
...
function OpsRow({
  ...
  meta: string;
  href: string;
  accent: "approval" | "activity" | "automation";
}) {
  return (
...
function CompactEmpty({ children }: { children: React.ReactNode }) {
  return (
    <div className="mission-ops-empty">
      <span>{children}</span>
    </div>
  );
}
...
function CommandPalette({ onClose }: { onClose: () => void }) {
  const actions = [
    ...
    { label: "Open AI team", detail: "Agents and roles", href: "/team" },
    {
      ...
      detail: "Notes, processes and decisions",
      href: "/memory",
    },
    {
      label: "View reports",
      detail: "Execution and operational reporting",
      href: "/reports",
    },
  ];
  return (
...
function formatSchedule(value: string, recurrence?: string | null) {
  ...
  if (!Number.isFinite(date.getTime()))
    return recurrence && recurrence !== "one_off" ? recurrence : "Scheduled";
  ...
  return recurrence && recurrence !== "one_off"
